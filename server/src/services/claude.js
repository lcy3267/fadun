...
export async function generateGroups({ type, goal, desc, defendant }) {
  const prompt = [
    '你是一位中国民事诉讼律师助手，正在为当事人制定个性化的证据收集方案。',
    '',
    '【案件信息】',
    `案件类型：${type}`,
    `维权目的：${goal}`,
    ...
    '每个分组需包含：',
    '- group：分组名称，6 字以内',
    '- desc：该分组一句话说明，说明应收集什么材料',
    '- guide：具体搜集建议，告诉用户去哪里找、如何保存，2-3 句话，可提及具体 App 或平台',
    '',
...
export async function analyzeEvidence({ images, caseInfo }) {
  ...
  const prompt = [
    '你是一位专业的中国民事诉讼律师助手，正在帮助当事人分析证据材料。',
    '',
    '【案件信息】',
    `案件类型：${type}`,
    `维权目的：${goal}`,
    ...
    '',
    `【任务】`,
    `以上共 ${images.length} 张图片，请按顺序逐张分析，只返回 JSON 数组：`,
    '[',
    '  {',
    '    "valid": true,',
    '    "evType": "证据类型简称（4字以内）",',
    '    "group": "归属分类名称（从上方选一个，无关填null）",',
    '    "verdict": "对本案证明力的具体说明，不超过40字"',
    '  }',
    ']',
    '',
    `注意：数组长度必须等于图片数量（${images.length}）；verdict 要具体说明内容与案情的关联；只返回 JSON。`,
  ].join('\n')
...
export async function generateDocument({ caseData, evidenceList }) {
  ...
  const prompt = [
    '你是一位专业的中国民事诉讼律师，请根据以下信息起草一份正式的维权陈述书。',
    '',
    '【当事人信息】',
    ...
    '}',
    '',
    '要求：语言正式，逻辑清晰，每个 section content 不少于 80 字；建议行动路径给出分步骤的具体建议。',
  ].join('\n')
